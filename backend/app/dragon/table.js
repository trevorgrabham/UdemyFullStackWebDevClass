...
class DragonTable {
    static storeDragon(dragon) {
        return new Promise((resolve, reject) => {
            ...
            [dragon.birthdate, dragon.nickname, dragon.generationId, dragon.isPublic, dragon.saleValue, dragon.sireValue],
            (error, response) => {
    ...
    static getDragon({ dragonId }) {
        return new Promise((resolve, reject) => {
            pool.query(`SELECT birthdate, nickname, "generationId", "isPublic", "saleValue", "sireValue" 
                        FROM dragon 
                        WHERE dragon.id = $1`,
            [dragonId],
            (error, response) => {
